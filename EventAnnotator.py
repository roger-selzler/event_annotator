...
class Event_annotator():
    ...
    def __create_figure(self):
        ...
        self.fig,ax = plt.subplots(len(self.signals()),1,sharex=True)
        ...
        for signal_i,signal in enumerate(self.signals()):
            ...
            self._lines[signal_i].append(self.ax[signal_i].plot(signal.data.x,
                                                                ...
                                                                color='red',
                                                                alpha=0.7))
            self.ax[signal_i].set_title(signal.name)
    ...
    def __on_enter_axes(self,event):
        ...
        if self.config['log']: print(self._active_signal.name)
    ...
    def __rectangle_on_select(self,eclick,erelease):
        if self.config['log']:
            ...
            print("function __rectangle_on_select with signal '{}'".format(self._active_signal.name))            
    ...
    def add_signal(self,x,y=None,yf=None,name=''):
        ...
        self._signals[len(ea._signals)] = signal
    ...
    def summary(self):
        ...
        if len(self.signals())>0:
            print('\nSignals: ')
            for signal_i,signal in enumerate(self.signals()):
                print('{:>3}: {:<20}'.format(signal_i, signal.name))
            ...
            if label._label_type == 'points':
                ...
                if len(self._scatter[signal_i][label._label_index]) == 0:
                    ...
                    self._scatter[signal_i][label._label_index].append(self.ax[signal_i].scatter(dataf.x,
                                                                                                  dataf.y,
                                                                                                  color='black',
                                                                                                  marker=markers[np.mod(label._label_index,len(markers))],
                                                                                                  label=label._name
                                                                                                  ))
                    self._scatter[signal_i][label._label_index].append(self.ax[signal_i].scatter(dataf.x,
                                                                                                  dataf.yf,
...
class Label(object):
    def __init__(self,event_annotator,label_name,label_type='points',filename_loc=None):
        self.__ea = event_annotator
        self.__assert_label_type(label_type)
        self.__assert_label_name(label_name)       
        self.__assert_filename_loc(filename_loc)
        self._label_type = label_type
        self._name = label_name
        self._filename_loc = filename_loc
...
class Main_window(QtWidgets.QWidget,object):
    ...
    def initUI(self):
        ...
        wdg.setLayout(main_box)
        self.main_area.addWidget(wdg)
        # self.main_area.addLayout(main_box)
        self.main_tool_layout = QtWidgets.QVBoxLayout() 
        # placeholder = QtWidgets.QWidget()
        # placeholder.setLayout(self.main_tool_layout)
        # placeholder.setFixedWidth(50)
        # self.main_tool_layout.setSizeConstraint(QtWidgets.QLayout.SizeConstraint(10))
        main_right_box = QtWidgets.QVBoxLayout()
        # main_right_box.setStretch(0,0.5)
        for box in [main_box,self.main_tool_layout,main_right_box]:
            box.setAlignment(QtCore.Qt.AlignTop)
            
        wdg = QtWidgets.QWidget(self)
        wdg.setStyleSheet('background-color: black;')
        wdg.setLayout(self.main_tool_layout)
        self.main_tool_layout.setStretch(0,1)
        main_box.addWidget(wdg)
        # main_box.addLayout(self.main_tool_layout)
        self.main_area_widget=QtWidgets.QWidget(self)
        self.main_area_widget.setStyleSheet('background-color: red;')
        self.main_area_widget.setLayout(main_right_box)
        main_right_box.setStretch(0,1)
        main_box.addWidget(self.main_area_widget)
        # main_box.addLayout(main_right_box)
        ...
        for signal_to_ploti,signal_to_plot in enumerate(['original', 'filtered', 'both']):
            ...
            btn.setMaximumWidth(self.max_btn_width)
        ...
        self.label_layout = QtWidgets.QVBoxLayout(self)
        self.main_tool_layout.addLayout(self.label_layout)
        self._button_group_labels = QtWidgets.QButtonGroup()
        self._button_group_labels.buttonClicked[int].connect(self.__label_on_click)
        ...
        self.figure_toolbar = mpl.backends.backend_qt5agg.NavigationToolbar2QT(self.figure_canvas,self)
        self.ax = self.figure_canvas.figure.subplots(3,1)
        for axi,ax in enumerate(self.ax):
            ax.plot(np.random.random(100)+axi)
        self.figure_canvas.figure.tight_layout()
        
        main_right_box.addWidget(self.figure_toolbar)
        main_right_box.addWidget(self.figure_canvas)
    ...
    def __label_on_click(self,id):
        self.__ea.active_label = self.__ea.label(id)
        self.app.update_available_tools()
    ...
    def add_label_button(self,label):
        btn = QtWidgets.QPushButton(label._name)
        btn.setCheckable(True)
        btn.setMaximumWidth(self.max_btn_width)
        if label == self.__ea.active_label: btn.setChecked(True)
        else: btn.setChecked(False)
        self._button_group_labels.addButton(btn,label._label_index)
        self.label_layout.addWidget(btn)
...
class Add_label(QtWidgets.QWidget,object):
    ...
    def __add_label(self):
        ...
        if filename != '':
            if not os.path.isdir(fileloc):
                error_dialog.showMessage('Invalid folder location')
                return
        if filename == '' or fileloc == '':
            buffer.write('label has no filename or location and data will not be saved.\n')
            filename_loc='' 
        else:
            filename_loc = os.path.join(fileloc,filename)
        label = self.__ea.add_label(label_name,label_type,filename_loc)
        self.app.add_label_button(label)
    ...
    def __get_directory(self):
        ...
        # self.edit_fileloc.setText(directory.replace('/','//'))
